...
- **Use of `@IROnlyFactories`**: This annotation is required on the graph and extension declarations if you need to generate providers, as well as binding container declarations. This is because external metro extensions will be run in one-pass with other metro native FIR extensions, and those providers won't be seen by metro, so we need to defer the factory generation to the IR phase.
