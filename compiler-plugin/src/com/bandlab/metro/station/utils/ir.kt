 ...
 */
internal fun generateProvideParamBody(
    ...
    val initialParamProperty =
        firstParamClass.declarations.filterIsInstance<IrProperty>().find {
            it.name == Ids.initialParamName
        }
    if (initialParamProperty != null) {
        ...
        declaration.body =
            DeclarationIrBuilder(pluginContext, declaration.symbol).irBlockBody {
                val getInitialParam =
                    irCall(initialParamGetter).apply {
...
internal fun generateProvideBaseTypeBody(
    pluginContext: IrPluginContext,
    declaration: IrSimpleFunction,
) {
...
internal fun generateProvideParamFlowBody(
    pluginContext: IrPluginContext,
    declaration: IrSimpleFunction,
) {
    val regularParams = declaration.parameters.filter { it.kind == IrParameterKind.Regular }
    ...
    val createParamFlowFunction =
        providerClass.functions.firstOrNull { it.name.asString() == "createParamFlow" } ?: return
